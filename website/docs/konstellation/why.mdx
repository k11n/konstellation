...
The inspiration for Konstellation comes from having worked at two companies that have taken different paths to adopting Kubernetes. At Evie Labs, my previous startup, we struggled to adopt Kubernetes due to the learning curve, and used Kubernetes raw. It was painful for developers to use, having to hand map YAML resources was extremely error prone. We lacked the resources to invest in proper tooling. At Medium, we've invested in an impressive set of internal solutions that made Kubernetes practical to developers, but yet those tools remained internal. Having been through the same experience twice, I wanted an open source set of tools that can solve these pain points; but none existed.
