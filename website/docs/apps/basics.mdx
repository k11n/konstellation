...
In Konstellation, ports are required to have a name, and those names are used to reference that port in Ingress as well as [app dependencies](services#connecting-to-services).
...
If the app needs to be surfaced to the internet, you'd want to declare an [Ingress](../reference/manifest#ingressconfig) in order to create a load balancer.
