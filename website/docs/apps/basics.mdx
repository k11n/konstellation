...
You'll get a load balancer address (check with `kon app status <yourapp>`). Set up an ALIAS or CNAME with your DNS provider and traffic will start flowing through to your app.
...
Konstellation supports autoscaling out of the box, and will manage a [Horizontal Pod Autoscaler](https://kubernetes.io/docs/tasks/run-application/horizontal-pod-autoscale/) based on your configuration. Depending on the specified resource usage, your app will scale up and down in the number of pods to meet demand.
...
Konstellation could can take full advantage of IAM roles when running apps. By default, all of your apps are ran with the same role as the EKS node, which are fairly restrictive.

Kubernetes supports role based access control (RBAC) with Service Accounts. It works wonderfully in handling the identity of your app automatically. However, Service Accounts are designed to control access within Kubernetes itself, and is not connected to the AWS role in any way.

In order to make this work simply in a reproducible manner, Konstellation supports an extension of service accounts: [Linked Service Account](../reference/linkedserviceaccount) that links IAM policies to a Kubernetes service account.

Once you've created a Linked Service Account, set it in your app manifest and your app would be able to use the policies set.
