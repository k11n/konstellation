...
Target is a concept in Konstellation that provides a namespace for apps. A target is roughly equivalent to a specific environment of an app. For example, you could specify a production and a development target, with different configurations for hostnames, scale parameters, and receive target-specific configs.
...
A release is a base unit of an app's deployment. It locks in the app's build along with any configurations. Each change in the app's build or config would trigger a new release to be created. You could list the releases with `kon app status <yourapp>`
...
If the app needs to be surfaced to the internet, you'd have to declare an [ingress](../reference/manifest#ingressconfig) in order to create a load balancer.
...
Once created, you can check the address of the load balancer with `kon app status <yourapp>`. Then, set up an ALIAS or CNAME with your DNS provider pointing the domain to that address.
