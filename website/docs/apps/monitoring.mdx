...
The default setup could provide basic metrics observable from outside of apps, but to measure app-specific metrics, you need to integrate one of the Prometheus [client libraries](https://prometheus.io/docs/instrumenting/clientlibs/) and expose a metric endpoint on one of the ports. You can enable Prometheus scraping by setting the [prometheus]() field in the manifest.
