...
This Prometheus install is configured to collect metrics for the cluster and Istio mesh, including data that it could observe about your apps.

### Collecting app metrics

The default setup could provide basic metrics observable from outside of your app, but to measure app-specific attributes, you need to integrate one of the Prometheus [client libraries](https://prometheus.io/docs/instrumenting/clientlibs/) and expose a metric endpoint on one of your ports (by default `/metrics`). You can enable Prometheus scraping by setting the [prometheus]() section of your app manifest.
...
...
  prometheus:
    endpoints:
    - port: http
      interval: 10s  # s, m suffixes
      # override path if you aren't able to surface metrics on the default /metrics path
      # path: /metrics
```

Konstellation uses [Prometheus Operator](https://github.com/coreos/prometheus-operator) and will set up ServiceMonitors for your app. The above setup will instruct Prometheus to scrape your app's `http` port every 10 seconds.
